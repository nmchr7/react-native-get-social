...
UI_CONFIG="EMPTY"
AUTOREGISTER_PUSH=false
FOREGROUND_NOTIFICATIONS=false
USE_UI=true
DEBUG=true
AUTO_INIT=true
IGNORE_COCOAPODS=false
...
while [ "$1" != "" ]; do
	...
	case $PARAM in
		...
		--foreground-notifications)
			FOREGROUND_NOTIFICATIONS=$VALUE
			;;
		--ignore-cocoapods)
			IGNORE_COCOAPODS=$VALUE
			;;
		*)
...
python $INSTALLER_SCRIPT_DIR/installer.py --app-id $APP_ID \
	...
	--foreground-notifications $FOREGROUND_NOTIFICATIONS \
	--debug $DEBUG \
	--auto-init $AUTO_INIT \
	--ignore-cocoapods $IGNORE_COCOAPODS

#cleanup
`rm -f $LATEST_INSTALLER_VERSION`
`rm -f getsocial-installer-script.zip`
`rm -f frameworks.zip`
